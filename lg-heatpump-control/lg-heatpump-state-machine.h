        ...
        bool check_change_events(){
          ...
          for(it = events.begin(); it != events.end(); it++){
            if(*it == DEFROST_RUN){
              ...
              if(!input[RELAY_HEAT]->state){
                //relay_heat switched off. Check if thermostat_sensor still on
                //this happens for example when thermostat switches off during SWW run and then back on again
                if(input[THERMOSTAT_SENSOR]->state){
                  id(relay_heat).turn_on();
