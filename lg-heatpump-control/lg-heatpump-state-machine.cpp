  input_struc::input_struct(uint_fast32_t* run_time_pointer){
    run_time = run_time_pointer;
  }
  void input_struc::receive_state(bool new_state){
    state = new_state;
    if(new_state != prev_state)  input_change_time = *run_time;
  }
  void input_struc::receive_value(float new_value){
    value = new_value;
    if(new_value != prev_value) input_change_time = *run_time;
  }
  uint_fast32_t input_struc::seconds_since_change(){
    return *run_time - input_change_time;
  }
  bool input_struc::has_flag(){
    return (prev_value != value || prev_state != state);
  }
  void input_struc::unflag(){
    prev_state = state;
    prev_value = value;
  }
  ...
  const char * state_machine_class::state_friendly_name(states stt = NONE){
  ...
  const char * state_machine_class::state_name(states stt = NONE){
    if(stt == NONE) stt = current_state;
  ...
  //*******************Backup Heat*********************************
  //***************************************************************
  void state_machine_class::backup_heat(bool mode,bool cop_limit_trigger = false){
    if(mode){
